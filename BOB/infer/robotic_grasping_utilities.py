...
class RobotController:
    ...
    def execute_trajectory(self, trajectory: list):
        ...
        for i, angle in enumerate(target_angles):
            ...
            joint_angle.value = unnormalize_angle(angle)
        ...
        try:
            ...
            while True:
                ...
                max_error = 0
                # Around line 683-687 — replace this block:
                for i, (t, c_obj) in enumerate(zip(target_angles, current_angles)):
                    c_raw = c_obj.value
                    c = normalize_angle(c_raw)   # ← ADD THIS: bring to [-180, 180]
                    diff = (t - c + 180) % 360 - 180
                    error = abs(diff)
                    
                    if error > max_error:
                        max_error = error
                    
                    # Only print if we are "stuck"
                    if error > 10: 
                        print(f"Joint {i+1} stuck: Target {t:.2f}, Current {c:.2f}, Error {error:.2f}")
    ...
    def move_and_close(self, target_pose, target_width_m: float = 0.01, MAX_OPENING_M: float = 0.1, speed_mps: float = 0.01) -> bool:
        """
